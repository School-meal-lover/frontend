...
//-----------------------------🔥TimeSelector🔥-----------------------------
//조식, 중식 석식 선택하는 Component
function TimeSelector(){
  const [timeSelected, setTimeSelected] = useState("breakfast");

  const times =[
    {name: "breakfast", time: "조식 (08:00 ~ 09:00)"},
    {name: "lunch", time: "중식 (11:30 ~ 13:30)"},
    {name: "dinner", time: "석식 (17:00 ~ 18:30)"}
  ]

  return(
    <div className="m-18 flex gap-4 justify-center items-center">
      {times.map((time) => (
        <div key={time.name} onClick={() => setTimeSelected(time.name)}
        className={`flex justify-center items-center w-full max-w-[260px] aspect-[260/48] rounded-md font-semibold border border-orange-400 hover:bg-[#FFB080] transition
         ${timeSelected === time.name ? "bg-[#FFB080]" : "bg-white"}`}>
        {time.time}
        </div>
      ))}
    </div>
