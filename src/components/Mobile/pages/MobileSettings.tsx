...
export default function MobileSettings() {
    ...
    return (
        <div className="bg-[#F8F4F1] min-h-screen p-5">
            <div className="max-w-2xl mx-auto">
                {/* 제목 */}
                <h1 className="text-2xl font-bold text-gray-900 mb-6">{t('settings.title')}</h1>

                {/* 언어 설정 카드 */}
                <div className="bg-white rounded-xl shadow-sm border border-orange-200 p-6 mb-4">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-orange-200 p-6 mb-4">
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <span className="font-medium text-gray-700">{t('notifications.enable')}</span>
                        <button
                            onClick={handleToggleEnabled}
                            disabled={pwaInstalled !== 'installed' || notificationPermission !== 'granted'}
                            className={`relative inline-flex h-7 w-14 items-center rounded-full transition-colors ${settings.enabled ? 'bg-orange-500' : 'bg-gray-300'
                        ...
                        >
                            <span
                                className={`inline-block h-5 w-5 transform rounded-full bg-white transition-transform ${settings.enabled ? 'translate-x-8' : 'translate-x-1'
                                    }`}
                            />
                        </button>
                    </div>
                    ...
                    {settings.enabled && (
                        <div>
                            <h3 className="font-semibold text-gray-700 mb-3">{t('notifications.mealsTitle')}</h3>

                            {(['breakfast', 'lunch', 'dinner'] as MealType[]).map((mealType) => (
                                <div key={mealType} className="mb-4 p-3 bg-gray-50 rounded-lg">
                            ...
                            <div className="flex gap-3">
                                <label className="flex items-center cursor-pointer flex-1 p-3 bg-gray-50 rounded-lg">
                                    <input
                                        type="checkbox"
                                        ...
                                        className="w-5 h-5 text-orange-500 rounded focus:ring-orange-500 mr-3"
                                    />
                                    <span className="font-medium text-gray-700">{t('notifications.weekdays')}</span>
                                </label>
                                <label className="flex items-center cursor-pointer flex-1 p-3 bg-gray-50 rounded-lg">
                                    <input
                                        type="checkbox"
                                        ...
                                        className="w-5 h-5 text-orange-500 rounded focus:ring-orange-500 mr-3"
                                    />
                                    <span className="font-medium text-gray-700">{t('notifications.weekends')}</span>
                                </label>
                            </div>
                        </div>
                    )}
